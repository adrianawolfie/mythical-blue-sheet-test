...
(function () {
  ...
  function ensureModal(kind) {
    ...
    document.body.insertAdjacentHTML('beforeend',`<div id="${id.modal}" class="library-picker-backdrop" hidden><section class="library-picker-modal" role="dialog" aria-modal="true" aria-labelledby="${kind}LibraryPickerTitle"><div class="library-picker-header"><div><div class="sb-hdr" id="${kind}LibraryPickerTitle">${cfg.title}</div><p>${cfg.description}</p></div><button type="button" class="spell-picker-close" onclick="closeSrdLibraryPicker('${kind}')" aria-label="Close ${cfg.title.toLowerCase()} picker">×</button></div><div class="library-picker-toolbar"><label><span>Search</span><input id="${id.search}" placeholder="${cfg.search}"></label><label><span>Category</span><select id="${id.category}"><option value="all">All Categories</option></select></label></div><div class="library-picker-layout"><div id="${id.results}" class="library-picker-results"></div><aside id="${id.preview}" class="library-picker-preview" aria-live="polite"><p class="library-picker-placeholder">Select an entry to preview its rules text before adding it.</p></aside></div><div class="library-picker-footer"><span id="${id.count}"></span><button type="button" class="add-btn" onclick="addCustomFromSrdPicker('${kind}')">${cfg.customLabel}</button></div></section></div>`);
